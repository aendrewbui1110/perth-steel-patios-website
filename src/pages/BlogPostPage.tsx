...
export default function BlogPostPage() {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="pt-32 pb-10 bg-[#131311] border-b border-[#1A1917]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-xs text-[#5E5E58] mb-8 flex-wrap">
            <Link to="/" className="hover:text-[#C8713A] transition-colors">Home</Link>
            <ChevronRight size={12} />
            <Link to="/blog" className="hover:text-[#C8713A] transition-colors">Blog</Link>
            <ChevronRight size={12} />
            <span className="text-[#858580] line-clamp-1">{post.title}</span>
          </nav>
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <span className="inline-block text-[10px] font-bold text-[#C8713A] uppercase tracking-[0.2em] bg-[#C8713A]/10 px-2.5 py-1 rounded">
                {categoryLabel}
              </span>
              <span className="text-xs text-[#5E5E58]">{formattedDate}</span>
              <span className="flex items-center gap-1 text-xs text-[#5E5E58]">
                <Clock size={11} />
                {post.readTime}
              </span>
            </div>
      ...
      {/* Placeholder Image */}
      <section className="bg-[#0C0C0F]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="aspect-[21/9] bg-gradient-to-br from-[#1C1A18] to-[#151412] border border-[#22211E] rounded-xl flex items-center justify-center">
      ...
      <section className="py-16 bg-[#0C0C0F]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            {post.content ? (
              <div
                className="prose prose-invert prose-lg max-w-none text-[#858580] leading-relaxed"
                dangerouslySetInnerHTML={{ __html: post.content }}
              />
            ) : (
              <div className="bg-[#1C1A18] border border-[#22211E] rounded-xl p-10 text-center">
                <p className="text-[#858580] text-lg mb-2">Full article coming soon.</p>
                <p className="text-[#5E5E58] text-sm">
              ...
              {relatedPosts.map((related, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <div className="p-5">
                      <span className="inline-block text-[10px] font-bold text-[#C8713A] uppercase tracking-[0.2em] bg-[#C8713A]/10 px-2.5 py-1 rounded mb-2">
                        {blogCategories.find((c) => c.slug === related.category)?.name ?? related.category}
                      </span>
                      <h4 className="text-[#EAE6DF] font-bold text-sm leading-snug group-hover:text-[#C8713A] transition-colors duration-200">
                        {related.title}
                      </h4>
                      <div className="flex items-center gap-2 text-xs text-[#3E3D38] mt-2">
                        <Clock size={11} />
                        {related.readTime}
                      </div>
                    </div>
