    ...
    <>
      ...
      <section className="pt-32 pb-16 bg-[#131311] border-b border-[#1A1917]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-2 text-xs text-[#5E5E58] mb-8">
            ...
            <span className="hover:text-[#C8713A] transition-colors">Areas</span>
            <ChevronRight size={12} />
            <span className="text-[#858580]">{suburb.name}</span>
          </nav>
          ...
          >
            ...
            <p className="text-lg text-[#858580] leading-relaxed mt-6 max-w-3xl">
              {suburb.description}
            </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            >
              ...
              <p className="text-[#858580] leading-relaxed mb-6">
                {suburb.description}
              </p>
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-sm">
                  <Building size={16} className="text-[#C8713A] flex-shrink-0" />
                  <span className="text-[#5E5E58]">Council:</span>
                  <span className="text-[#858580]">{suburb.council}</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <Compass size={16} className="text-[#C8713A] flex-shrink-0" />
                  <span className="text-[#5E5E58]">Distance from CBD:</span>
                  <span className="text-[#858580]">{suburb.distanceFromCBD}</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <MapPin size={16} className="text-[#C8713A] flex-shrink-0" />
                  <span className="text-[#5E5E58]">Region:</span>
                  <span className="text-[#858580]">{suburb.region}</span>
                </div>
              </div>
            </motion.div>
                ...
                {suburb.popularStyles.map((style) => {
                  ...
                  return (
                    <Link
                      ...
                      className="group flex items-center justify-between bg-[#1C1A18] border border-[#22211E] rounded-lg p-4 hover:border-[#C8713A]/50 transition-all duration-300"
                    >
                      <div>
                        ...
                        {service && (
                          <p className="text-[#5E5E58] text-xs mt-1 line-clamp-1">
                            {service.description}
                          </p>
                        )}
                      </div>
                      <ArrowRight
                        size={14}
                        className="text-[#C8713A] flex-shrink-0 group-hover:translate-x-1 transition-transform duration-200"
                      />
                    </Link>
                  );
                })}
          ...
          {suburbProjects.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {suburbProjects.map((project, index) => (
                <motion.div
                  key={project.id}
                  className="bg-[#1C1A18] border border-[#22211E] rounded-xl overflow-hidden"
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.4, delay: index * 0.1 }}
                >
                  <div className={`${project.aspect} bg-[#151412]`}>
                    <img
                      src={project.image}
                      alt={project.title}
                  ...
                  <div className="p-5">
                    <span className="text-[10px] font-bold text-[#C8713A] uppercase tracking-[0.2em]">
                      {project.type}
                    </span>
                    <h4 className="text-[#EAE6DF] font-bold mt-1">{project.title}</h4>
                    <p className="text-[#5E5E58] text-sm mt-1">{project.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="bg-[#1C1A18] border border-[#22211E] rounded-xl p-10 text-center">
              <p className="text-[#858580] mb-6">
                We've completed multiple projects in {suburb.name}. Contact us to see examples
                and discuss your build.
              </p>
              <Link
                to="/contact"
                className="inline-flex items-center gap-2 bg-[#C8713A] hover:bg-[#B5632E] text-white px-6 py-3 rounded font-bold text-xs uppercase tracking-widest transition-colors duration-200"
              >
          ...
          {suburbTestimonials.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {suburbTestimonials.map((testimonial, index) => (
                <motion.div
                  key={testimonial.id}
                  className="bg-[#1C1A18] border border-[#22211E] rounded-xl p-8"
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.4, delay: index * 0.1 }}
                >
                  <div className="flex gap-1 mb-4">
                    {Array.from({ length: testimonial.rating }).map((_, i) => (
                      <span key={i} className="text-[#C8713A] text-sm">&#9733;</span>
                    ))}
                  </div>
                  <p className="text-[#858580] leading-relaxed mb-4 italic">
                    "{testimonial.text}"
                  </p>
                  <div className="text-sm">
                    <span className="text-[#EAE6DF] font-bold">{testimonial.name}</span>
                    <span className="text-[#5E5E58]"> &middot; {testimonial.location} &middot; {testimonial.date}</span>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <section className="py-20 bg-[#0C0C0F]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            className="bg-[#1C1A18] border border-[#22211E] border-l-4 border-l-[#C8713A] rounded-xl p-8"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
          >
            ...
            <p className="text-[#858580] leading-relaxed mb-2">
            ...
            <Link
              to="/process"
              className="inline-flex items-center gap-2 text-[#C8713A] text-sm font-bold uppercase tracking-widest hover:gap-3 transition-all duration-200"
            >
