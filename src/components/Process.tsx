...
export function Process() {
  return (
    <section id="process" className="py-24 bg-[#141418]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        {/* Header */}
        <div className="text-center max-w-3xl mx-auto mb-20">
          <h2 className="text-xs font-bold text-[#D4622A] uppercase tracking-[0.2em] mb-3">Our Process</h2>
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-11 left-0 w-full h-px bg-[#22222A] z-0" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
            {steps.map((step, index) => (
              ...
              >
                {/* Icon circle */}
                <div className="w-22 h-22 w-[88px] h-[88px] bg-[#1A1A20] border-2 border-[#D4622A]/40 rounded-full flex items-center justify-center mb-6 shadow-lg shadow-black/40 group-hover:border-[#D4622A] group-hover:bg-[#D4622A]/10 transition-all duration-300">
                  <step.icon size={30} className="text-[#D4622A]" />
                </div>

                {/* Step number — faint behind */}
                <div className="absolute top-0 right-0 -mr-3 -mt-3 font-heading text-6xl font-bold text-[#D4622A]/8 -z-10 select-none">
                  {step.number}
                </div>
                ...
                <p className="text-[#858590] leading-relaxed text-sm">{step.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
